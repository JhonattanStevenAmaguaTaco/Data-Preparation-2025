...
emgBasePath = '.\data\New-Dataset\Users\emgs\'; % Path to EMG data
indicesBasePath = '.\data\New-Dataset\Users\idxs\'; % Path to indices data
outputBasePath = '.\data\New-Dataset\OutputUsers\'; % Path to save processed output
...
for userIdx = 1:length(userFolders)
    ...
    % Generar la subcarpeta específica para el usuario según userIdx
    userFolderName = sprintf('user%d', userIdx); % Crear nombre de carpeta, e.g., "user1"
    userFolderPath = fullfile(resultBasePath, userFolderName);
    
    % Crear la subcarpeta del usuario si no existe
    if ~exist(userFolderPath, 'dir')
        mkdir(userFolderPath);
    end
    
    % Guardar el archivo en la subcarpeta específica
    outputFilePath = fullfile(userFolderPath, 'newUserData.mat');
